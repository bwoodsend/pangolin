# Welcome to pangolin's test suite!
...
pangolin aims to be out-the-box compatible with
...
that PyInstaller does need special help when compiling pangolin
then that *help* goes in a *hook* file (see
`pangolin/__pyinstaller/hook-pangolin.py`).
...
changed the pangolin source code.
...
This executable is just `pytest.main()` with pangolin
...
Otherwise you may unintentionally use the original source pangolin
...
If you've modified either the source code or the hooks for pangolin
